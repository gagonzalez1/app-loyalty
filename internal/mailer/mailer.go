...
func PasswordResetMessage(appURL, to, token string) model.EmailMessage {
	...
	content := fmt.Sprintf(`
        ...
        %s`,
		html.EscapeString(absoluteURL(appURL, "/email-assets/mr-puntazo.png")),
...
func BrandInvitationMessage(appURL, to, token string, details BrandInvitationDetails) model.EmailMessage {
	...
	content := fmt.Sprintf(`
        ...
        %s`,
		html.EscapeString(absoluteURL(appURL, "/email-assets/mr-puntazo.png")), brandVisual, html.EscapeString(brandName),
